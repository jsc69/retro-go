...
namespace AMeteor
{
	...
	{
		// int size = odroid_sdcard_read_file(filename, (char*)m_rom, ROM_SIZE);
		std::ifstream file(filename);
		std::memset(m_rom, 0, ROM_SIZE);
		file.read((char*)m_rom, ROM_SIZE);
		if (file.bad())
			return false;
		return true;
	}
	...
	{
		uint32_t until = std::min(size, ROM_SIZE);
		std::memcpy(m_rom, data, until);
		std::memset(m_rom+until, 0, ROM_SIZE-until);
	}
...
#define memmem memmem_compat
	...
	void Memory::WriteCart (uint16_t add, uint8_t val)
	{
		...
		if (!m_cart)
			if (add == 0x5555)
			{
				if (memmem((char*)m_rom, ROM_SIZE, "FLASH1M_V", 9))
					SetCartType(CTYPE_FLASH128);
				else
					SetCartType(CTYPE_FLASH64);
			}
